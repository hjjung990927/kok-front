...
## ✍️ 총평
#### ⏳ 어려웠던 부분
프로젝트를 진행하면서 기술적으로 가장 어려웠던 부분은 파일 업로드/조회, 권한 제어, 캐싱 처리, 그리고 REST 기반 비동기 설계였다. <br>
특히 AWS S3 도입 과정에서는 Presigned URL 개념 이해와 null 예외 처리, S3 Key 관리가 쉽지 않았고 <br>
Spring Security 적용 단계에서는 권한 흐름이 명확하지 않아 예상치 못한 에러를 자주 마주했다. <br>
또한, 조회 성능 강화를 위해 캐싱을 적용했지만, 좋아요 기능과 정합성이 맞지 않는 문제가 발생해 로직을 재구성하기도 했다.

#### 🛠️ 해결 방법
▶ AWS S3 Presigned URL 문제 해결

FileDTO 누락이 원인임을 파악하고, 게시글 파일과 매핑되는 데이터를 모두 저장하도록 로직을 수정 <br>
Presigned URL의 TTL(Time to Live) 및 접근 정책을 조절하며 보안과 편의성을 균형 있게 유지
...
#### ✨ 느낀 점
이번 프로젝트를 통해 단순한 CRUD 수준을 넘어, "실제로 서비스가 돌아가기 위한 백엔드 흐름" 을 깊게 이해할 수 있었다. <br>
AWS, Security, Cache, Presigned URL, Redis 등 실전에서 많이 쓰이는 기술들을 직접 맞부딪히며 구현해본 경험 덕분에, 오류를 해결하는 과정 자체가 큰 자산이 되었다고 느낀다. <br>
또한, 기능을 만드는 것보다 데이터 정합성, 보안, 예외 처리, 사용자 경험, 유지 보수성이 훨씬 중요하다는 것을 깨달았고, 앞으로의 코드들도 이러한 관점에서 설계해야 한다는 기준이 생겼다.
